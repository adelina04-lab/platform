...
## Что сайт сейчас собирает и чем это регулируется

Ничего. На странице нет ни формы, ни аналитики, ни партнёрских ссылок —
только телефон владельца. Телефон в подвале персональных данных посетителя
не собирает, поэтому обязанностей оператора персональных данных сейчас не
возникает: ни уведомления в Роскомнадзор, ни политики, ни согласий.

Готовая форма обратной связи и политика обработки данных лежат в
[`apps/web/src/drafts/`](apps/web/src/drafts/) вместе с PHP-обработчиком в
`deploy/drafts/` — код дописан и проверен на живом сервере, ждёт только
юридической части.

### Что включает счётчик обязанностей

Каждая строчка срабатывает не от планов, а от того, что реально появилось
на странице.

| Что добавляем | Что становится обязательным |
|---|---|
| Форму или любое поле ввода контактов | уведомление в Роскомнадзор **до** начала сбора, опубликованная политика, отдельная неотмеченная галочка согласия, HTTPS |
| Яндекс Метрику или другую аналитику | то же самое: аналитика обрабатывает IP и cookie посетителя |
| Первую партнёрскую ссылку | пометка «Реклама», данные рекламодателя, токен `erid` из ОРД, отчёт в ЕРИР |
| Первый доход от партнёрок | обязательные отчисления 3 % с дохода от интернет-рекламы (с 1 апреля 2025, ежеквартально) |

Отдельно, до подачи заявок в партнёрские программы: НПД не применяется к
доходу по агентскому договору, договору поручения и комиссии — пункт 5 части
2 статьи 4 закона 422-ФЗ. Договор на оказание рекламных услуг применяется.
Оферту каждой программы нужно прочитать заранее: если она агентская, под неё
потребуется ИП.

Свой домен и HTTPS нужны раньше всего остального — без них партнёрские
программы обычно не принимают заявку.
