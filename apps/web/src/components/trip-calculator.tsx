    ...
    <div className="flex flex-col gap-14 lg:gap-20">
      ...
      <div className="grid items-start gap-12 lg:grid-cols-[1.04fr_0.96fr] lg:gap-10">
        ...
        <div className="relative">
          {/* Плашки остались фоном: сквозь стекло они читаются как цветная
              глубина и дают сцене объём. */}
          <GlassScene parts={parts} variant="edges" className="absolute inset-0 hidden lg:block" />

          <div className="glass relative z-10 rounded-modal p-5 sm:p-7 lg:p-8">
            <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 border-b border-paper/15 pb-4">
            ...
            <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
              <span className="font-mono text-[10.5px] uppercase tracking-[0.12em] text-paper/45">
                Уровень
              </span>
              <div
                role="group"
                aria-label="Уровень комфорта"
                className="flex gap-1 rounded-field bg-paper/10 p-1"
              >
                {TIERS.map((t) => (
                  <button
                    key={t}
            ...
            <ul className="mt-4 text-paper">
              {active.components.map((c) => (
                <li
                  key={c.component}
                  className={`flex items-start gap-2 border-b border-paper/10 py-2.5 ${
                    c.included ? "" : "opacity-40"
                  }`}
                >
                  <span className="flex min-w-0 flex-col gap-0.5">
                    <span className="flex items-center gap-2 text-[14.5px] font-semibold leading-tight">
                      <span
                        aria-hidden
                    ...
                    <span className="pl-[18px] text-[11.5px] leading-snug text-paper/45">
                      {c.note}
                    </span>
                  </span>
            ...
            <div className="mt-6 flex flex-wrap items-end justify-between gap-x-6 gap-y-2 border-t-2 border-paper/25 pt-5">
              <span className="flex flex-col gap-1">
                <span className="font-display text-[19px] font-extrabold tracking-[-0.03em] text-paper">
                  Итого
                </span>
                <span className="text-[12.5px] leading-snug text-paper/45">
              ...
              <span
                aria-live="polite"
                className="tnum font-display text-[clamp(30px,4.2vw,48px)] font-extrabold leading-[0.9] tracking-[-0.045em] text-cold"
              >
                {formatMoney(shownTotal)}
              </span>
            </div>

            <dl className="mt-5 grid gap-x-6 gap-y-2 border-t border-dashed border-paper/20 pt-4 sm:grid-cols-3">
              {footNotes.map((f) => (
                <div key={f.label} className="flex items-baseline justify-between gap-3">
                  <dt className="font-mono text-[10px] uppercase tracking-[0.1em] text-paper/45">
                    {f.label}
                  </dt>
                  <dd className="tnum text-[13.5px] font-bold text-paper">{f.value}</dd>
                </div>
              ))}
            </dl>

            <p className="mt-4 text-[11.5px] leading-snug text-paper/35">
